...
BUILTIN_RULES: list[VulnRule] = [
    ...
    VulnRule(
        ...
        severity=Severity.CRITICAL,
        languages=["php"],
        pattern=r"""(?:mysql_query|mysqli_query|pg_query)\s*\(|(?:"|')\s*(?:SELECT|INSERT|UPDATE|DELETE).*?(?:"|')\s*\.\s*\$""",
        description="检测到直接执行 SQL 查询或使用字符串拼接构造 SQL 语句",
    ...
    VulnRule(
        rule_id="SAST-007",
        cwe="CWE-89",
        title="SQL 注入 (Go fmt.Sprintf)",
        severity=Severity.CRITICAL,
        languages=["go"],
        pattern=r"""fmt\.Sprintf\s*\(\s*"(?:SELECT|INSERT|UPDATE|DELETE).*?%""",
        description="Go: 使用 fmt.Sprintf 构造 SQL 语句，若拼入用户输入则构成 SQL 注入",
        remediation="使用参数化查询 (?, $1 等)",
        confidence="High",
    ),
    VulnRule(
        rule_id="SAST-008",
        cwe="CWE-89",
        title="SQL 注入 (Rust format!)",
        severity=Severity.CRITICAL,
        languages=["rust"],
        pattern=r"""format!\s*\(\s*"(?:SELECT|INSERT|UPDATE|DELETE).*?\{""",
        description="Rust: 使用 format! 宏构造 SQL 语句，若拼入用户输入则构成 SQL 注入",
    ...
    VulnRule(
        rule_id="SAST-014",
        cwe="CWE-78",
        title="OS 命令注入 (Go exec)",
        severity=Severity.HIGH,
        languages=["go"],
        pattern=r"""exec\.Command\s*\(\s*(?:cmd|command|shell)\s*""",
        description="Go: exec.Command 使用变量作为命令名，可能导致命令注入",
        remediation="避免使用变量作为命令名，或使用白名单校验",
        confidence="Medium",
    ),
    VulnRule(
        rule_id="SAST-015",
        cwe="CWE-78",
        title="OS 命令注入 (Rust Command)",
        severity=Severity.HIGH,
        languages=["rust"],
        pattern=r"""Command::new\s*\(\s*(?:cmd|command|shell|input)\s*\)""",
        description="Rust: Command::new 使用变量作为命令名，可能导致命令注入",
        remediation="避免使用变量作为命令名，或使用白名单校验",
        confidence="Medium",
    ),
    VulnRule(
        rule_id="SAST-016",
        cwe="CWE-78",
        title="OS 命令注入 (Java Runtime)",
        severity=Severity.CRITICAL,
        languages=["java"],
        pattern=r"""Runtime\.getRuntime\(\)\.exec\s*\(\s*(?:cmd|command|str|input|\"sh\"\s*\+\s*|\"cmd\.exe\"\s*\+\s*)""",
        description="Java: Runtime.exec 执行动态拼接的命令",
        remediation="使用 ProcessBuilder 并将参数分开传递",
        confidence="High",
    ),
    VulnRule(
        rule_id="SAST-C-001",
        cwe="CWE-120",
        title="缓冲区溢出 (strcpy/strcat)",
        severity=Severity.CRITICAL,
        languages=["cpp"],
        pattern=r"""(?:strcpy|strcat|sprintf|gets)\s*\(""",
        description="C/C++: 使用了不安全的字符串操作函数，可能导致缓冲区溢出",
        remediation="使用 strncpy, strncat, snprintf 等安全函数",
        confidence="High",
    ),
    VulnRule(
        rule_id="SAST-C-002",
        cwe="CWE-78",
        title="OS 命令注入 (system/popen)",
        severity=Severity.CRITICAL,
        languages=["cpp"],
        pattern=r"""(?:system|popen)\s*\(\s*(?:cmd|command|buf|str|input)""",
        description="C/C++: system/popen 执行变量命令，可能导致命令注入",
        remediation="避免直接执行 Shell 命令，或严格过滤输入",
        confidence="Medium",
    ),
    VulnRule(
        rule_id="SAST-C-003",
    ...
    VulnRule(
        rule_id="SAST-CS-001",
        cwe="CWE-89",
        title="SQL 注入 (C# String Concat)",
        severity=Severity.CRITICAL,
        languages=["csharp"],
        pattern=r"""(?:SqlCommand|SqlDataAdapter|OdbcCommand|OleDbCommand)\s*\(\s*(?:".*"\s*\+|.*\+\s*".*")""",
        description="C#: 使用字符串拼接构造 SQL 命令，可能导致 SQL 注入",
        remediation="使用 SqlParameter 进行参数化查询",
        confidence="High",
    ),
    VulnRule(
        rule_id="SAST-CS-002",
        cwe="CWE-89",
        title="SQL 注入 (C# Format)",
        severity=Severity.CRITICAL,
        languages=["csharp"],
        pattern=r"""(?:SqlCommand|SqlDataAdapter|OdbcCommand|OleDbCommand)\s*\(\s*string\.Format""",
        description="C#: 使用 string.Format 构造 SQL 命令，可能导致 SQL 注入",
        remediation="使用 SqlParameter 进行参数化查询",
        confidence="High",
    ),
    VulnRule(
        rule_id="SAST-CS-003",
        cwe="CWE-78",
        title="OS 命令注入 (C# Process)",
        severity=Severity.HIGH,
        languages=["csharp"],
        pattern=r"""Process\.Start\s*\(\s*(?:cmd|command|input|str)""",
        description="C#: Process.Start 直接执行变量命令，可能导致命令注入",
        remediation="避免直接执行用户输入的命令，或使用 ProcessStartInfo 严格限制参数",
        confidence="Medium",
    ),
    VulnRule(
        rule_id="SAST-CS-004",
        cwe="CWE-611",
        title="XXE (C# XmlDocument)",
        severity=Severity.HIGH,
        languages=["csharp"],
        pattern=r"""(?:XmlDocument|XmlTextReader)\s*\(\s*.*\).*DtdProcessing\s*=\s*DtdProcessing\.(?:Parse|Prohibit)?""",
        description="C#: XML 解析可能未禁用 DTD 处理，导致 XXE",
        remediation="设置 DtdProcessing = DtdProcessing.Prohibit 或 Ignore",
        confidence="Low",
    ),
    VulnRule(
        rule_id="SAST-CS-005",
        ...
        severity=Severity.HIGH,
        languages=["csharp"],
        pattern=r"""(?:File\.Open|File\.ReadAllText|File\.ReadAllBytes|File\.WriteAllText)\s*\(\s*(?:path|file|filename|input)""",
        description="C#: 文件操作直接使用变量作为路径，可能导致路径遍历",
